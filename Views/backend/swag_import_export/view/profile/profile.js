...
Ext.define('Shopware.apps.SwagImportExport.view.profile.Profile', {
        ...
        me.formPanel = Ext.create('Ext.form.Panel', {
            ...
            fillForm: function () {
                ...
                if (node.get('type') === 'attribute') {
                    this.child('#swColumn').getStore().load({
                        params: {
                    ...
                    if (!isFirst) {
                        this.child('#parentKey').getStore().load({
                            params: {
                                profileId: me.profileId,
                                adapter: node.get('adapter')
                            }
    ...
    /**
     * On focus reload the comboStore
     *
     * @param { Shopware.form.field.PagingComboBox } combo
     */
    onFocusProfileComboBox: function (combo) {
        var me = this;

        me.profilesStore.reload({
            callback: function (result) {
                if (result.length == 0) {
                    combo.picker.pagingToolbar.movePrevious();
                }
            }
        });
    }
